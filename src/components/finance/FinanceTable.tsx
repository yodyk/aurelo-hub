...
export function FinanceTableShell({ children, total, filtered, search, onSearch, onClear }: { children: ReactNode; total: ReactNode; filtered: boolean; search: string; onSearch: (value: string) => void; onClear: () => void }) {
  const scrollRef = useRef<HTMLDivElement>(null);
  const [dragging, setDragging] = useState(false);
  const drag = useRef({ x: 0, scroll: 0 });
  const onDown = (event: MouseEvent<HTMLDivElement>) => { if ((event.target as HTMLElement).closest('button,a,input,[role="menu"]')) return; setDragging(true); drag.current = { x: event.clientX, scroll: scrollRef.current?.scrollLeft || 0 }; };
  ...
  return <div className="border border-[var(--hairline)] bg-card min-w-0">
    <div className="flex flex-wrap items-center gap-2 border-b border-[var(--hairline)] p-3">
      ...
      {filtered && <Button variant="ghost" size="sm" onClick={onClear}><X /> Clear All Filters</Button>}
    </div>
